...
from universal_mcp_airtable.app import Airtable

@pytest.fixture
def app_instance():
    """Provides a Airtable instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return Airtable(integration=mock_integration)

def test_universal_mcp_airtable_app_initialization(app_instance):
    """
    Test that the Airtable instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "airtable", "Airtable instance has unexpected name."


def test_universal_mcp_airtable_tool_docstrings_format(app_instance):
    """
    Test that each tool method in Airtable has a well-formatted docstring,
...
def test_universal_mcp_airtable_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in Airtable is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
